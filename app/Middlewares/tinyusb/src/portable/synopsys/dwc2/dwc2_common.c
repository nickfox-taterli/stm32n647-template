...
static bool reset_core(dwc2_regs_t* dwc2) {
  ...
  if ((gsnpsid & DWC2_CORE_REV_MASK) < (DWC2_CORE_REV_4_20a & DWC2_CORE_REV_MASK)) {
    ...
    timeout = DWC2_RESET_TIMEOUT_LOOPS;
    while ((dwc2->grstctl & GRSTCTL_CSRST) && --timeout) {}
    TU_VERIFY(timeout != 0);
  } else {
    ...
    timeout = DWC2_RESET_TIMEOUT_LOOPS;
    while (!(dwc2->grstctl & GRSTCTL_CSRST_DONE) && --timeout) {}
    TU_VERIFY(timeout != 0);
    dwc2->grstctl = (dwc2->grstctl & ~GRSTCTL_CSRST) | GRSTCTL_CSRST_DONE;
  }

  timeout = DWC2_RESET_TIMEOUT_LOOPS;
  while (!(dwc2->grstctl & GRSTCTL_AHBIDL) && --timeout) {} // wait for AHB master IDLE
  return timeout != 0;
}
